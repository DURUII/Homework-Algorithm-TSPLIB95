...
##### 模拟退火 
事实上，人们从物理世界状态演化、自然界各种现象、千百年来生存斗争经验获得启发，以仿生拟人拟物途径设计了各种千奇百怪五花八门的算法。由于模拟退火（Simulated Annealing）具有自然背景，实现简单，故没有逃避使用此算法。
...
> 受《求解二维矩形Packing问题的一种优美度枚举算法》的启发，对于最近邻而言，进行如下改进：我们在生成的所有路线中，取前m条进行2-opt随机化改进（这是一种非随机型优化算法），如果超过指定时间T，算法停机。

步骤二：依照模拟退火算法通用步骤，执行有限次邻域搜索，概率跳坑。

> 邻域包括插入（将某个城市从全排列中删除，再插入任意一个位置）、块插入（将任意连续k个城市从全排列删除，再在尾部一齐插入）、对换（将第i号和第j号交换）、以及2-opt（主要是选取两条边，进行重组，可有效消除交叉）。模拟退火的初始排列状态不是纯随机的，而是在步骤一所经历的排列集合中选取若干个还算合理，有希望的解空间初始点经过一次轻微扰动的结果。设置超时算法停机。

实现过程中，有以下思考：
1. 贪婪算法在2-opt优化后可以获得不小的提升。以a280为例，最近邻贪婪算法得到的结果是`2975`，而经过优化后可达`2710`。
2. 克里斯托菲德斯算法饱含人类智慧。
3. 有学者认为，淬火模拟的是个一成不变因而往往不贴切的物理世界，我们应当向比晶体有更高智慧的人学习，这是不无道理的。以a280为例，在步骤一非随机型近似算法的基础上，模拟退火邻域搜索鲜有突破。

### experimental setup 实验设置

算法用`Python3.9`编程实现；测试编码同步进行。在`3.30GHz`的`AMD-Ryzen 9-6900HS`个人微型计算机上，本文选取在`EUC_2D`类型且城市数小于等于1000中全部48个benchmark测试用例。

- 每个问题实例计算`10`次
- 最近邻：2-opt优化停机时间`0.5`秒
- 模拟退火：停机时间为`50`秒，初始温度`10e6`，终止温度`10e-6`，衰减系数`0.99`。

### experimental results 实验结论

用48个来自TSPLIB的问题实例进行测试，给出了其中`?`个实例的最优解：进步空间巨大。
